...
pro_name <- c("nki31_p11")
raw_path <- paste0("./map_seg_output/", pro_name)
keep_gene <- c("SHC1","AKT3", "FHIT","PIK3CA","EGFR","MYC", "CDKN2A", "GATA3","PTEN", "CCND1", "PAK1", "CDK4",
               "FOXA1", "TP53", "ERBB2", "BRCA1", "AURKA")
keep_gene2 <- c("CKS1B", "FHIT","CCNE2","MYC", "PAK1", "BRCA2","RB1", "ERBB2", "BCL2", "AURKA")
...
name_meta <- rownames(varbin_mtx_tumor@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(dispense = stringr::str_extract(my_name, "yes|no"), peak = ifelse(stringr::str_detect(my_name, "_d_"), "d", 
         ...
         timepoint = ifelse(stringr::str_detect(my_name, "_nki31p_"), "primary", "recurrence")) 
...
library(ComplexHeatmap)
mtx_srt <- as.data.frame(varbin_mtx_tumor_log2@colData) %>% arrange(factor(subclones, 
                                  levels = paste0("c", 1:length(unique(varbin_mtx_tumor_log2@colData$subclones)))))
...
ploidy_pri <- 3.297949084
ploidy_rec <- 2.712880327
...
pdf(paste0("./figures/", pro_name, "_complexHeatmap_consensus_integer_with_barplot.pdf"), height = 4, width = 9)
