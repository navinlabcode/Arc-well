...
pro_name <- c("duke248_p3")
raw_path <- paste0("./map_seg_output/", pro_name)
keep_gene <- c("FHIT", "PIK3CA", "MYC", "PGR", "BRCA2", "RB1", "TP53", "ERBB2", "BRCA1")
keep_gene2 <- c("FHIT","PIK3CA", "PPP2R2A", "MYC","CCND1", "BRCA2", "RB1")
...
name_meta <- rownames(varbin_mtx_tumor@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(dispense = stringr::str_extract(my_name, "yes|no"), peak = ifelse(stringr::str_detect(my_name, "_d_"), "d", 
                                                        ifelse(stringr::str_detect(my_name, "_a_"), "a", "nopeakinfo")),
         timepoint = ifelse(stringr::str_detect(my_name, "_duke_24_"), "primary", 
                            ifelse(stringr::str_detect(my_name, "_duke_25_"), "recurrence", "notimepoint"))) %>% 
...
varbin_mtx_tumor_log2 <- varbin_mtx_tumor_log[, !(subclones == "c0" | 
                                                    (varbin_mtx_tumor_log@colData$tp_clst %in% clone_num_less3$comb))]
...
ha_bottom = columnAnnotation(df = cs, col = list(my_sample=c("sCNA"="grey70", "neu" = "grey92","cCNA"="purple2")), 
                             clonal_state = anno_mark(at=gene_bin_sel_sample$rowname, labels = gene_bin_sel_sample$gene, side = "bottom", 
                                                      labels_gp = gpar(fontsize = 14, col = gene_bin_sel_sample$my_color)))

ha_row2=rowAnnotation(df = anno_mtx[,3:2], col = list(subclones= clst_col, timepoint=tp_col), show_annotation_name = F, 
...
treeplt2 <- ggtree::inset(treeplt, pies, width=0.2, height=0.2)
treeplt2
cowplot::ggsave2(paste0("./figures/", pro_name, "_medicc2_tree.pdf"), treeplt2, width = 4.5, height = 1.75)
...
# dir.create("./objects/cs_bin_class/")
...
pdf(paste0("./figures/", pro_name, "_complexHeatmap_consensus_integer_pca_rca_merged.pdf"), height = 5, width = 9)
