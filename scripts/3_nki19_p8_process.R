...
keep_gene <- c("SHC1","AKT3", "PIK3CA", "PPP2R2A", "CDKN2A","CDK4", "MDM2", "BRCA2", "RB1", "TP53", "ERBB2", "CCNE1", "AURKA")
keep_gene2 <- c("CDKN2C","FGFR1", "CCNE2", "MYC", "PGR")
...
name_meta <- rownames(varbin_mtx_tumor@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(dispense = stringr::str_extract(my_name, "yes|no"), 
         ...
         timepoint = ifelse(stringr::str_detect(my_name, "_19p_"), "primary", 
                            ifelse(stringr::str_detect(my_name, "_19re_"), "recurrence", "notimepoint"))) %>% 
...
mean((meta_mtx %>% dplyr::filter(timepoint == "recurrence"))$median_bin_count)
...
all_name_meta <- rownames(varbin_mtx@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(peak = ifelse(stringr::str_detect(my_name, "_d_"), "d", ifelse(stringr::str_detect(my_name, "_a_"), "a", "nopeakinfo")),
         timepoint = ifelse(stringr::str_detect(my_name, "_nki19p_"), "primary", 
                            ifelse(stringr::str_detect(my_name, "_nki19re_"), "recurrence", "notimepoint"))) %>% 
...
#-----Single cell Ratio plot----#####
cellname <- c("sc132_nki19p_r45c54yes_s_19p_a_w_b1_s3295_r1_001")
cellname <- janitor::make_clean_names(cellname)

varbin_mtx_tumor_log2 <- readRDS(paste0("./objects/", pro_name, c("_final_filtered_overdisp_integerCN_copykit.rds")))

#---preparing x axis--
chrom.lengths <- c(249250621, 243199373, 198022430, 191154276, 180915260, 171115067, 159138663, 146364022, 
...
chrom.names <- c(paste0("chr", 1:22),"chrX")
...
chrom.rects <- data.frame(xstart = chrom.abspos.start, xend = chrom.abspos.end)
xbreaks <- rowMeans(chrom.rects)
chrom.rects$colors <-c(rep(c("white", "gray"), 11), "white")

#chromosomes squares background
ggchr.back <- list(geom_rect(data = chrom.rects, aes(xmin = xstart, xmax = xend, ymin = -Inf, ymax = Inf, fill = colors), alpha = .2), 
                   scale_fill_identity()) 

sec_breaks <- c(0, 0.5e9, 1e9, 1.5e9, 2e9, 2.5e9, 3e9)
sec_labels <- c(0, 0.5, 1, 1.5, 2, 2.5, 3)

#---get ratio and integer data---
cell_bin_ratio <- varbin_mtx_tumor_log2@assays@data$ratios[, ..cellname]
cell_seg_ratio <- varbin_mtx_tumor_log2@assays@data$segment_ratios[, cellname]
cell_int_seg_ratio <- varbin_mtx_tumor_log2@assays@data$integer[, cellname]

#----color dots with ploidy---
ploidy_cell <- round(varbin_mtx_tumor_log2@colData[cellname,]$ploidy_used)
int_color0 <- pals::ocean.balance(length(0:(2*ploidy_cell))) %>% str_replace( "#F0ECEB" ,"grey75")

if(length(int_color0)>=max(cell_int_seg_ratio)){
  int_color <- int_color0
  names(int_color) <- 0:(2*ploidy_cell)
} else {
  int_color <- c(int_color0, rep(int_color0[length(int_color0)],times = (max(cell_int_seg_ratio) - 2*ploidy_cell)))
  names(int_color) <- 0:max(cell_int_seg_ratio)
}

#----make matrix--
ratio_mtx <- cbind(varbin_mtx_tumor_log2@rowRanges$abspos, cell_bin_ratio, cell_seg_ratio, cell_int_seg_ratio) %>% 
  dplyr::rename(chr_abs="V1", cell_ratio = all_of(cellname)) %>% 
  dplyr::mutate(int_col = plyr::mapvalues(cell_int_seg_ratio, from = as.numeric(names(int_color)), to = int_color))

#---integer color---
p1 <- ggplot(ratio_mtx) + geom_point(aes(x = chr_abs, y = cell_ratio, color = factor(cell_int_seg_ratio)), 
                                     shape = 20, size = 0.5, alpha = .7) +
  scale_color_manual(values = int_color[names(table(ratio_mtx$cell_int_seg_ratio))]) +
  geom_line(aes(chr_abs, cell_seg_ratio), col = "black", size = 0.5) + ggchr.back + 
  scale_x_continuous(breaks = xbreaks, labels = gsub("chr", "", chrom.names), position = "top", expand = c(0,0),
                     sec.axis = sec_axis(~., breaks = sec_breaks, labels = sec_labels, name = "Genome Position (Gb)")) +
  scale_y_continuous(limits = c(0, 3.5), sec.axis = sec_axis(~ . * ploidy_cell, name = "integer copy number")) +
  theme_classic() + ggtitle(paste0(pro_name,": ",cellname, " ploidy: ",ploidy_cell)) + 
  theme(axis.title.x.top = element_blank(), legend.position = "right") +
  guides(color = guide_legend(override.aes = list(size=5, shape = 16)))
p1
p2 <- ggrastr::rasterize(p1, layers='Point', dpi=350)
cowplot::ggsave2(paste0("./figures/", pro_name,"_",cellname, "_ratio_plot.pdf"), p2, width = 8, height = 2.5)


#----with subclone 20 but without bad cells----
varbin_mtx_tumor_log2 <- readRDS(paste0("./objects/", pro_name, c("_final_filtered_overdisp_copykit.rds")))
no_c20_names <- rownames(varbin_mtx_tumor_log2@colData %>% as.data.frame() %>% 
                           dplyr::filter(!(subclones == "c20" & timepoint == "recurrence")))
varbin_mtx_tumor_log2 <- varbin_mtx_tumor_log2[,no_c20_names]
...
saveRDS(varbin_mtx_tumor_log2, file = paste0("./objects/", pro_name, c("_with_c20_but_nobadcell_final_filtered_overdisp_integerCN_copykit.rds")))
# varbin_mtx_tumor_log2 <- readRDS(paste0("./objects/", pro_name, c("_with_c20_but_nobadcell_final_filtered_overdisp_integerCN_copykit.rds")))
...
treeplt2 <- ggtree::inset(treeplt, pies, width=0.08, height=0.08)
treeplt2
cowplot::ggsave2(paste0("./figures/", pro_name, "_medicc2_tree.pdf"), treeplt2, width = 4.5, height = 2.75)
...
clst_col_cs <- new_pal[1:length(table(varbin_mtx_tumor_log2@colData$subclones))]
subclone_cs <- paste0("c", 1:length(table(varbin_mtx_tumor_log2@colData$subclones)))
...
pdf(paste0("./figures/", pro_name, "_complexHeatmap_consensus_integer_pca_rca_merged.pdf"), height = 6, width = 9)
