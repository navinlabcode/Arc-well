...
#-----correlation with block ages--(Figure 2E)---######
...
write.csv(cna_num, "./metrics/number_cna_events_with_chr_level_events.csv")
# cna_num <- read.csv("./metrics/number_cna_events_with_chr_level_events.csv", row.names = 1)
...
p1
cowplot::ggsave2("./figures/CNA_events_number_in_allsamples_updated.pdf", p1, width = 3.75, height = 4)
...
p1 <- ggplot(mpd2, aes(x=timepoint, y=values)) + geom_line(aes(group=sample), linetype = "dashed", colour = "grey", size = 0.5) + 
  geom_point(aes(color=sample), size=4) + theme_bw() + scale_color_manual(values =  sample_col) + theme(axis.title = element_blank())
p1
cowplot::ggsave2("./figures/singlecell_MPD_in_allsamples.pdf", p1, width = 3.75, height = 3.75)
...
# colnames(mpd_downsam_res) <- c("Timepoint", "nCells", "singlecell_mpd","sample")
sample_col <- c("#565FAB","#CF3D32","#5DB1DD","#812268","#78C36A","#486A84","#BB6438","#F0E584","#D595A7","#749B59")
...
#-----# CNA, #subclone, MPD vs clinical features-(Figure 3F)----######
meta_mtx <- read.csv("./metrics/clinical_metadata_table.csv")
din_age_meta <- read.csv("./metrics/din_block_ages.csv", row.names = 1)
sclone <- read.csv("./metrics/subclone_number.csv", header = T)
cna_num <- read.csv("./metrics/number_cna_events_with_chr_level_events.csv", row.names = 1)
...
write.csv(subclo_cna_meta, "./metrics/paired_clin_comp_meta_updated.csv")
# subclo_cna_meta <- read.csv("./metrics/paired_clin_comp_meta_updated.csv", row.names = 1)

subclo_cna_meta2 <- subclo_cna_meta %>% mutate(PR = na_if(PR, "Unk"), HER2 = na_if(HER2, "Unk"), 
                                               histology = ifelse(histology == "DCIS", "DCIS", "IBC"))
write.csv(subclo_cna_meta2, "./metrics/paired_clin_comp_meta_NA_updated.csv")
# subclo_cna_meta2 <- read.csv("./metrics/paired_clin_comp_meta_NA_updated.csv", row.names = 1)
...
cowplot::ggsave2("./figures/MPD_NumCNA_tp_er_pr_her2_his_grade2_updated.pdf", p_list, width = 15, height = 6, limitsize = F)
...
#---PreCR test---###
precr <- read.csv("./metrics/normalize_preCR.csv", header = F)
precr2 <- precr %>% mutate(normalized_amount=V2/3680)
precr2

p1 <- ggplot(data=precr2, aes(x=V1, y=normalized_amount)) + geom_bar(stat="identity", fill="grey50") + theme_bw() + 
  theme(axis.title.x = element_blank())
p1
cowplot::ggsave2("./figures/preCR_normalized_amount.pdf", p1, width = 3, height = 3)


#-----# CNA in PCA and RCA (Figure 6A)----####
pca_rca <- read.csv("./metrics/PCA_RCA_CNA_events.csv", row.names = 1)
a <- wilcox.test(pca_rca$PCA_MRCA, pca_rca$RCA_MRCA, paired = T)
pca_rca_col <- c("#7f3f97", "#056e38")

pca_rca2 <- pca_rca %>% rownames_to_column() %>% dplyr::select("rowname","PCA_MRCA", "RCA_MRCA") %>% reshape2::melt(id  = c("rowname")) %>% 
  mutate(group=rep(1:8, times= 2))
colnames(pca_rca2) <- c("sample", "timepoint", "num_CNA","group")

p1 <- pca_rca2 %>% mutate(sample = factor(sample, levels = sample_name)) %>% 
  ggplot(aes(y=num_CNA, x = sample, group = group)) + geom_line(aes(group = group)) + geom_point(aes(color=timepoint), size=4) +
  theme_bw() + scale_color_manual(values =  pca_rca_col) + ggtitle(paste0("p-value = ", a$p.value))
cowplot::ggsave2("./figures/num_CNA_of_PCA_vs_RCA.pdf", p1, width = 6, height = 2)

p2 <- pca_rca2 %>% mutate(sample = factor(sample, levels = rev(sample_name))) %>% 
  ggplot(aes(x=num_CNA, y = sample, group = group)) + geom_line(aes(group = group)) + geom_point(aes(color=timepoint), size=4) +
  theme_bw() + scale_color_manual(values =  pca_rca_col) + ggtitle(paste0("p-value = ", a$p.value))
p2
cowplot::ggsave2("./figures/num_CNA_of_PCA_vs_RCA_vertical.pdf", p2, width = 4, height = 4)
