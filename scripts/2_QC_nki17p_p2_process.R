...
keep_gene <- c("PIK3CA","CDKN1A","ESR1","EGFR","FGFR1","MYC","CDKN2A","PTEN","CCND1",
               "PGR","CDK4","BRCA2","RB1","TP53","BRCA1","BCL2","CCNE1","AURKA")
...
name_meta <- rownames(varbin_mtx_tumor@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(dispense = stringr::str_extract(my_name, "yes|no"), 
         peak = ifelse(stringr::str_detect(my_name, "_dpeak_"), "d", ifelse(stringr::str_detect(my_name, "_apeak_"), "a", "nopeakinfo")),
         timepoint = ifelse(stringr::str_detect(my_name, "_nki17p_"), "primary", 
                            ifelse(stringr::str_detect(my_name, "_nki17re_"), "recurrence", "notimepoint"))) %>% 
...
#-----event matrices and integer copy number--(from: Kris)---####
## calculate comp ploidy
ploidy_pri <- 3.236311068
...
all_name_meta <- rownames(varbin_mtx@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(peak = ifelse(stringr::str_detect(my_name, "_dpeak_"), "d", ifelse(stringr::str_detect(my_name, "_apeak_"), "a", "nopeakinfo")),
         timepoint = ifelse(stringr::str_detect(my_name, "_nki17p_"), "primary", 
                            ifelse(stringr::str_detect(my_name, "_nki17re_"), "recurrence", "notimepoint"))) %>% 
