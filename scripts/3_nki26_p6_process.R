...
keep_gene <- c("AKT3", "PIK3CA", "EGFR", "FGFR1", "MYC", "CDKN2A", "GATA3", "CCND1","CDK4","MDM2", "BRCA2", 
               "RB1", "TP53", "ERBB2", "BRCA1", "AURKA")
keep_gene2 <- c("PIK3CA", "FGFR1", "CCNE2", "CDKN2A", "CDK4","MDM2", "BRCA2", "RB1", "CCNE1","AURKA")
...
name_meta <- rownames(varbin_mtx_tumor@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(dispense = stringr::str_extract(my_name, "yes|no"), 
         ...
         timepoint = ifelse(stringr::str_detect(my_name, "_nki26p_"), "primary", 
                            ifelse(stringr::str_detect(my_name, "_nki26re_"), "recurrence", "notimepoint"))) 
...
varbin_mtx_tumor_log2 <- varbin_mtx_tumor_log[, !(subclones == "c0" | (varbin_mtx_tumor_log@colData$tp_clst %in% 
                                                                         clone_num_less3$comb))]
...
ploidy_pri <- 3.606741573
ploidy_rec <- 3.073398025
...
meta_con <- varbin_mtx_tumor_log2@colData %>% as.data.frame() %>% dplyr::select(subclones, timepoint) %>% 
  group_by(subclones, timepoint) %>%
  summarise(n=n()) %>% spread(timepoint, n) 
meta_con[is.na(meta_con)] <- 0
meta_con_p <- meta_con %>% mutate(sum=primary+recurrence) %>% 
  mutate(primary=primary/sum, recurrence=recurrence/sum) %>% dplyr::select(-sum) %>% as.data.frame()
...
ha_row_cs=rowAnnotation(subclones = my_order, col = list(subclones = clst_col_cs), show_annotation_name = F)
...
cellname <- c("SC132_NKI26P_R20C65YesS_26P_A_W_C2_S1506")
...
ggchr.back <- list(geom_rect(data = chrom.rects, 
                             aes(xmin = xstart, xmax = xend, ymin = -Inf, ymax = Inf, fill = colors), alpha = .2), 
                   scale_fill_identity()) 
