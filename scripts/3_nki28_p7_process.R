...
keep_gene <- c("SHC1","AKT3", "FHIT", "PIK3CA", "FGFR4","ESR1","EGFR", "PPP2R2A","CCNE2","GATA3", "CCND1","BRCA2",
               "RB1", "TP53", "ERBB2", "BRCA1", "BCL2", "AURKA")
keep_gene2 <- c("AKT3", "FHIT", "FGFR4","CCNE1")
...
name_meta <- rownames(varbin_mtx_tumor@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(dispense = stringr::str_extract(my_name, "yes|no"), peak = ifelse(stringr::str_detect(my_name, "_d_"), "d", 
         ...
         timepoint = ifelse(stringr::str_detect(my_name, "_nki28p_"), "primary", 
                            ifelse(stringr::str_detect(my_name, "_nki28re_"), "recurrence", "notimepoint"))) 
...
ploidy_pri <- 3.139105553
ploidy_rec <- 3.484904946
...
treeplt2 <- ggtree::inset(treeplt, pies, width=0.07, height=0.07)
cowplot::ggsave2(paste0("./figures/", pro_name, "_medicc2_tree.pdf"), treeplt2, width = 5, height = 4)
treeplt2 <- ggtree::inset(treeplt, pies, width=0.1, height=0.1)
cowplot::ggsave2(paste0("./figures/", pro_name, "_medicc2_tree.pdf"), treeplt2, width = 4.5, height = 2.5)
