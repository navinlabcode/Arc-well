...
pro_name <- c("nki15_p10")
raw_path <- paste0("./map_seg_output/", pro_name)
keep_gene <- c("AKT3", "FHIT", "EGFR", "FGFR1", "MYC", "CDKN2A", "GATA3","BRCA2", "RB1", "TP53", "ERBB2", "BRCA1", 
               "CCNE1", "AURKA")
keep_gene2 <- c("AKT3", "FHIT", "EGFR", "FGFR1", "MYC", "CDKN2A", "GATA3","BRCA2", "RB1", "TP53", "ERBB2", "BRCA1", 
                "CCNE1", "AURKA")
...
name_meta <- rownames(varbin_mtx_tumor@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(dispense = stringr::str_extract(my_name, "yes|no"), 
         ...
         timepoint = ifelse(stringr::str_detect(my_name, "_nki15p_"), "primary", 
                            ifelse(stringr::str_detect(my_name, "_nki15re_"), "recurrence", "notimepoint"))) %>% 
...
p2 <- ggplot(as.data.frame(varbin_mtx_tumor_log2@colData),aes(x = UMAP_1, y = UMAP_2, fill = timepoint)) + 
  ...
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1), axis.line = element_blank(), legend.text = element_text(size = 7))
p2
...
ploidy_pri <- 3.923176231
ploidy_rec <- 3.915846907
...
# medicc2 -a CN --total-copy-numbers -j 40 -vv input_file output_path
...
meta_con <- varbin_mtx_tumor_log2@colData %>% as.data.frame() %>% dplyr::select(subclones, timepoint) %>% 
  group_by(subclones, timepoint) %>% summarise(n=n()) %>% spread(timepoint, n) 
...
cowplot::ggsave2(paste0("./figures/", pro_name, "_medicc2_tree.pdf"), treeplt2, width = 4.5, height = 2.85)
...
pdf(paste0("./figures/", pro_name, "_complexHeatmap_consensus_integer_with_barplot.pdf"), height = 5.5, width = 9)
...
Heatmap(as.matrix(ht_all_mtx), cluster_columns = FALSE, border = TRUE, cluster_rows = TRUE, show_row_dend = FALSE, 
        row_split = anno_mtx$filter_state, name = "scheatmap", show_row_names = F, show_column_names = F, 
...
cellname <- c("sc71_8_r36c2yes_s_nki15re_a_peak_w_a1_s7779_sort_markdup")
cellname <- c("sc71_8_r36c2yes_s_nki15re_a_peak_w_a1_s7779")
...
#---integer color---
p1 <- ggplot(ratio_mtx) + geom_point(aes(x = chr_abs, y = cell_ratio, 
                                         color = factor(cell_int_seg_ratio)), shape = 20, size = 0.5, alpha = .7) +
