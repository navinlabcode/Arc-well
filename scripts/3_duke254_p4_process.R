...
keep_gene <- c("AKT3","FHIT", "FGFR4","ESR1", "FGFR1","MYC","PTEN","BRCA2", "RB1", "FOXA1","TP53","BRCA1","CCNE2","ZNF217","AURKA")
keep_gene2 <- c("CKDN2C","AKT3", "PIK3CA", "ESR1","EGFR", "CCNE2", "MYC", "GATA3", "CCND1", "CDK4","MDM2", "ERBB2", "CCNE1","AURKA")
...
name_meta <- rownames(varbin_mtx_tumor@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(dispense = stringr::str_extract(my_name, "yes|no"), peak = ifelse(stringr::str_detect(my_name, "_d_"), "d", 
         ...
         timepoint = ifelse(stringr::str_detect(my_name, "_duke_30_"), "primary", 
                            ifelse(stringr::str_detect(my_name, "31_"), "recurrence", "notimepoint"))) %>% 
...
ploidy_pri <- 2.767570401
ploidy_rec <- 4.15880088
...
meta_con <- varbin_mtx_tumor_log2@colData %>% as.data.frame() %>% dplyr::select(subclones, timepoint) %>% group_by(subclones, timepoint) %>%
...
Heatmap(as.matrix(ht_all_mtx), cluster_columns = FALSE, border = TRUE, cluster_rows = TRUE, show_row_dend = FALSE, 
        ...
        row_title = paste0("single cells: total: ",nrow(anno_mtx)," (",names(table(anno_mtx$filter_state)[1]), ": ", 
                           ...
                           "; ", names(table(anno_mtx$filter_state)[2]), ": ", table(anno_mtx$filter_state)[2], ")"), 
