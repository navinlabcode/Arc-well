...
pro_name <- c("nki12_p12")
raw_path <- paste0("./map_seg_output/", pro_name)

keep_gene <- c("FHIT", "PIK3CA", "FGFR1", "MYC", "CDKN2A", "PTEN", "CCND1", "BRCA2", 
               "RB1", "TP53", "ERBB2", "BRCA1", "CCNE1", "AURKA")
...
name_meta <- rownames(varbin_mtx_tumor@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(dispense = stringr::str_extract(my_name, "yes|no"), 
         ...
         timepoint = ifelse(stringr::str_detect(my_name, "_12p_"), "primary", "recurrence")) %>% 
   dplyr::select(c("dispense", "peak", "timepoint"))

varbin_mtx_tumor@colData <- cbind(varbin_mtx_tumor@colData, name_meta)
varbin_mtx_tumor2 <- varbin_mtx_tumor[,str_detect(varbin_mtx_tumor@colData$sample, "yes")]
varbin_mtx_tumor_log <- logNorm(varbin_mtx_tumor2, transform = "log2")
...
ploidy_pri <- 3.548724331
ploidy_rec <- 3.709368831
...
cowplot::ggsave2(paste0("./figures/", pro_name, "_medicc2_tree.pdf"), treeplt2, width = 4.5, height = 3)
...
pdf(paste0("./figures/", pro_name, "_complexHeatmap_consensus_integer_with_barplot.pdf"), height = 5, width = 9)
...
all_name_meta <- rownames(varbin_mtx@colData) %>% as.data.frame() %>% dplyr::rename(my_name = ".") %>%  
  mutate(peak = ifelse(stringr::str_detect(my_name, "_d_"), "d", ifelse(stringr::str_detect(my_name, "_a_"), "a", "nopeakinfo")),
         timepoint = ifelse(stringr::str_detect(my_name, "_12p_"), "primary", "recurrence")) %>% dplyr::select(c("peak", "timepoint"))
